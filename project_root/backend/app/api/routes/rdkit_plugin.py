...
@router.post("/", response_model=FilterPluginInDB)
async def create_filter(plugin: RDKitFilterCreate, db: AsyncSession = Depends(get_db)):
    response = await crud.create_plugin(db=db, plugin=plugin)
    return utils.get_plugin_response_model(response)

@router.get("/{plugin_id}", response_model=FilterPluginInDB)
...
@router.put("/{plugin_id}", response_model=FilterPluginInDB)
async def update_plugin(plugin_id: int, plugin: RDKitFilterUpdate, db: AsyncSession = Depends(get_db)):
    db_plugin = await crud.update_plugin(db=db, plugin_id=plugin_id, plugin=plugin)
    if db_plugin is None:
        raise HTTPException(status_code=404, detail="Plugin not found")
    return utils.get_plugin_response_model(db_plugin)

@router.delete("/{plugin_id}", response_model=FilterPluginInDB)
