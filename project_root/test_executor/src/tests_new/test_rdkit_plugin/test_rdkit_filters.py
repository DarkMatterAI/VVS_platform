...
from tests_new.utils.request_data import (
    generate_rdkit_item_request, 
    validate_response, 
    validate_api_response,
)
from tests_new.utils.rabbitmq_utils import rabbitmq_publish, poll_redis
from tests_new.utils.backend_utils import backend_execute_plugin, backend_delete_plugin
from tests_new.utils.db_utils import validate_item_checkin


TEST_SMILES = ['CC(C)c1cc(C(=O)NC[C@@H](O)c2ccccn2)n(C)n1']

@pytest.mark.asyncio
async def test_rdkit_filter_consumer(db_session, rabbitmq_connection, redis_connection, 
                                     backend_client, rdkit_test_filter):
