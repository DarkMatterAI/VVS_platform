...
class EmbeddingPluginExecutor(BasePluginExecutor):
    ...
    async def check_in_results(self, requests: List[ItemRequest], 
                            results: List[EmbedResponse]) -> Any:
        """Check in embedding results to database"""
        if self.db_persist:
            return await self.db_service.check_in_item_results(
            self.plugin, requests, results
        )
        return None 
...
class DataSourcePluginExecutor(BasePluginExecutor):
    ...
    async def check_in_results(self, requests: List[DataSourceRequest], 
                             results: List[DataSourceResponse]) -> Any:
        """Check in data source results to database"""
        return await self.db_service.check_in_data_source_results(
            self.plugin, requests, results, self.db_persist
        )
...
class FilterPluginExecutor(BasePluginExecutor):
    ...
    async def check_in_results(self, requests: List[ItemRequest], 
                            results: List[FilterResponse]) -> Any:
...
class ScorePluginExecutor(BasePluginExecutor):
    ...
    async def check_in_results(self, requests: List[ItemRequest], 
                            results: List[ScoreResponse]) -> Any:
...
class MapperPluginExecutor(BasePluginExecutor):
    ...
    async def check_in_results(self, requests: List[MapperRequest], 
                             results: List[MapperResponse]) -> Any:
...
class AssemblyPluginExecutor(BasePluginExecutor):
    ...
    async def check_in_results(self, requests: List[AssemblyRequest], 
                             results: List[AssemblyResponse]) -> Any:
